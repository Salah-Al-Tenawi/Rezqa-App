...
class AddFreelancerOfferController extends GetxController {
  var title = ''.obs;
  var description = ''.obs;
  var minPrice = ''.obs;
  var maxPrice = ''.obs;
 ...
 var selectedFiles = <Map<String, dynamic>>[].obs;
  ...
  try {
    ...
    if (response.data != null && response.data['data'] != null) {
      categories.value = List<Map<String, dynamic>>.from(response.data['data'])
          .where((category) =>
 ...
 Future<void> pickFiles() async {
    try {
      ...
      for (final file in files) {
        ...
        if (fileId != null) {
          selectedFiles.add({'id': fileId, 'name': file.name}); // تعديل هنا لإضافة معرف واسم الملف
        } else {
          Get.snackbar(
            'خطأ في رفع الملفات',
  ...
  Future<void> submitOffer() async {
    ...
    try {
      ...
      var response = await dioClient.post(
        "${ApiEndPoint.basrurl}${ApiEndPoint.submitOfferfrelancer}",
        data: data,
        options: Options(
          headers: {
            "Authorization":
